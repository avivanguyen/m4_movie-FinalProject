...
async function onSearchMovie(event) {
    ...
    const response = await fetch(`https://www.omdbapi.com/?apikey=5aa2e87d&s=${searchTerm}`);
    ...
    if (data.Response === "True") {
        ...
        movieContainer.innerHTML = data.Search.map(movie => movieCardHtml(movie)).join('');
    } else {
    ...
    <div class="movie-card">
        <div class="movie-card__container" onclick="toggleModal()">
            <div class="movie__poster">
                <img src="${movie.Poster !== 'N/A' ? movie.Poster : './assets/movie_poster-placeholder.png'}" alt="${movie.Title}">
            </div>
